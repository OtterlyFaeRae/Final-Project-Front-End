...
const SellStock = ({ price, stockToSell, user, cookies, setUser }) => {
  ...
  const sellStock = async () => {
    ...
    const stocksSold = await addStocks(
      stockToSell.label,
      stockToSell.label,
      -1 * parseFloat(input),
      parseFloat(price[0]),
        ...
        <StockCont>
          ...
          <BottomCont>
            <Input
              ...
              value={input}
            />
            <Button2 placeholder="Sell" onClick={handleClick}>
              Sell
            </Button2>
          </BottomCont>
        </StockCont>
      ...
      <Modal isOpen={modalOpen} style={customStyles} closeTimeoutMS={200}>
        {!purchaseSuccesful ? (
          <>
            ...
            <button className="stock-notSold" onClick={closeModalUnsuccessful}>
              CLOSE
            </button>
          </>
        ) : (
          <>
            ...
            <button className="stock-notSold" onClick={closeModalSuccessful}>
              CLOSE
            </button>
          </>
        )}
